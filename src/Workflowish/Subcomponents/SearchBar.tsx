...
export default (props: {
  ...
  return (
    <div style={{ display: "flex", padding: "10px 10px 0 10px" }}>
      <input
        ref={inputReference} // focus ref to change the focus
        placeholder={"🔍 Search"}
    ...
    while (nodeStack.length) {
      ...
      if (top.searchHighlight == "DFS_PENDING") {
        ...
        top.children.forEach(
          (child) => (child.searchHighlight = "DFS_PENDING")
        );
        ...
        if (top.data.toLowerCase().includes(searchText.toLowerCase())) {
          ...
          const shouldUncollapse = top.children.reduce(
            (shouldUncollapse, child) =>
              shouldUncollapse ||
